...
4. `src/structured-capture/ai.ts` 负责 AI 提取和 AI 接口测试。
5. `src/structured-capture/index.ts` 负责两条通道独立排队、独立容错。
6. `src/structured-capture/storage.ts` 负责 CSV 持久化和状态去重。
7. `src-tauri/src/structured_capture.rs` 负责高效追加写 CSV。
